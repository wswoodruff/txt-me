    ...
    value: {
        default: {
            command: async (srv, [to, message]) => {
                ...
                const statusListenerId = Math.random();
                const es = new EventSource(`https://wswoodruff.com/status/${statusListenerId}`);
                console.log(await twilioService.text(phoneNumber, to, message, statusListenerId));
